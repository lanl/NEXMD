...
! Local variables:
  ...
  if (ntwx .gt. 0) then
    ...
    if (remd_method .gt. 0) then
      ...
      call checkerror(nf90_def_var(mdcrd_ncid, 'temp0', nf90_double, &
                      (/ frame_dim_id /), temp_var_id), 'define temp0')
      call checkerror(nf90_put_att(mdcrd_ncid, temp_var_id, 'units', &
...
! Local variables:
  ...
  if (ord1 .eq. 1 .and. ord2 .eq. 2) then
    ...
    call checkerror(nf90_put_var(mdcrd_ncid, coord_var_id, crd(:,:), &
...
#ifdef MPI
  if (remd_method .gt. 0) then
    call checkerror(nf90_put_var(mdcrd_ncid, temp_var_id, temp0, &
                    start = (/ mdcrd_frame /)), 'write replica temperature')
  end if
#endif
#else
