...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!THIS SUBROUTINE IS THE STATE SPECIFIC SOLVENT SCF PROCEDURE
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
subroutine solvent_scf_and_davidson_test();
  ...
  implicit none
  _REAL_, dimension (:,:), allocatable::tmp,xi,xi_;
  _REAL_ f1,f2,cc;
  integer p,g;
  integer INFO;
  integer LWORK;
  logical check_symmetry;
  logical flag_,calc_Z;
  integer verbosity_save;
  _REAL_ :: OPTIMALSIZE;
  _REAL_ opt;
  INTEGER          LWMAX
  PARAMETER        ( LWMAX = 1000000000 )
...
!*     .. Intrinsic Functions ..
  ...
  if ((solvent_model.eq.3).or.(solvent_model.eq.5)) then
    call calc_cosmo_3();
  else if ((solvent_model.eq.2).or.(solvent_model.eq.4)) then
    call calc_cosmo_2();
  end if
  return
end subroutine
