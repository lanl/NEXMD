...
!Passed in
	  ...
	  _REAL_, intent(in) :: xisu(qm2ds%Nb*(qm2ds%Nb+1)/2) ! CML 7/13/12
!Local
...
! Local
      ...
      !qm2_Helect2 is a function
      _REAL_ qm2_helect2	! CML 7/13/12
...
! RCW: Caution, i and j reversed here.
   ...
   !call qm2_core_core_repulsion(iqm, jqm, rij, oneOverRij, RI, enuclr) !not used
    ...
    F(1:linear)=H(1:linear)
        ...
        ! below regarding UHF vs. RHF. Since W2Fock_atompair() is designed for SQM and for
        ! the ground state density matrix, it assumes that the DM, as represented in the MO
        ! basis, has 2 on the diagonal instead of 1, so the AO off-diagonal elements are also
        ! doubled. However, P (i.e. xi) is not created under this assumption, thanks to the
        ! adaptation from the old CEO code. The problem lies in that W2Fock_atompair() updates the F
        ! matrix with values which are HALF of what they should be. To counter this effect,
        ! F is divided by two before passing it in to the function. The function will replace
        ! the necessary values with half-magnitude values, to match the rest of the matrix.
        ! Once the function exits, the F matrix is then multipled by 2 to correct all the
...
!***********************************************************************
   ...
   do I=1,nminus
      K=K+1
      ED=ED+xisu(K)*0.5D0*(-hmatrix(K)+F(K))
      do J=1,I
         K=K+1
         qm2_helect2=qm2_helect2+xisu(K)*(-hmatrix(K)+F(K))
      end do
   end do
   ...
   qm2_helect2=qm2_helect2+ED


end function qm2_helect2
