...
!
   ...
   !=============================
   !  Calculation of QM (ground state) Forces
   !=============================

   use cosmo_C , only: potential_type,solvent_model
   use qm2_davidson_module
   use qmmm_module,only:qmmm_nml,qmmm_struct,qmmm_mpi
   use constants,only:zero,EV_TO_KCAL,KCAL_TO_EV
   ...
   if(qmmm_nml%qmtheory%DFTB) then
      ...
      !standard semi-empirical ground state forces
...
!ceps-Dielectric Permittivity from COSMO module
   ...
   if ((solvent_model.gt.0).and.(potential_type.eq.3)) call diegrd(qmmm_struct%dxyzqm) !adding COSMO gradient
   if(qmmm_mpi%commqmmm_master.and.qmmm_nml%verbosity>3) then
      ...
      write (6,'("!============================================================================")')
      write (6,'("! Forces in ground state with solvent for testing purposes only. ")')	
      write (6,'("! Vasyl Kuzmenko, July 2013 LANL")')
      write (6,'("!============================================================================")')		
      write (6,'("QMMM:")')
      write (6,'("QMMM: Forces on QM atoms from SCF calculation (KCal/mol)")')
