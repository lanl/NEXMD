...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
subroutine solvent_scf_and_davidson_test(qm2_struct,qm2ds,qmmm_struct);
    ...
    if (solvent_model.eq.3) then
        call calc_cosmo_3(qm2_struct,qm2ds,qmmm_struct);
    else if (solvent_model.eq.2) then
        call calc_cosmo_2(qm2_struct,qm2ds,qmmm_struct);
    end if
    return
end subroutine
...
!subroutines
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
subroutine calc_excsolven(qm2ds, qmmm_struct,energy)
    use qm2_davidson_module
...
!subroutines
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
subroutine calc_gssolven(qm2_struct, qm2ds,qmmm_struct,energy)
    use qm2_davidson_module
    use qmmm_module,only:qm2_structure
    ...
    call addfck(qmmm_struct, qm2ds%tz_scratch(1),qm2_struct%den_matrix);
...
! GS calculations
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
subroutine calc_xicommutator(qm2ds,V_potential)
    use qm2_davidson_module
...
!DAVIDSON
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
subroutine calc_cosmo_2(qm2_struct, qm2ds,qmmm_struct)
    use qm2_davidson_module
    use communism
    ...
    call davidson(qm2_struct,qm2ds, qmmm_struct);    !initial call in gas phase
    ...
    call calc_rhotz(qm2_struct,qm2ds,qmmm_struct,qmmm_struct%state_of_interest,qm2ds%rhoTZ,calc_Z);
    ...
    if(potential_type.eq.3) then !COSMO
        call VxiM(qm2ds,qm2ds%eta,v_solvent_difdens);
    elseif(potential_type.eq.2) then!ONSAGER
        call rcnfld(qm2_struct,qmmm_struct,v_solvent_difdens,qm2ds%eta,qm2ds%Nb)
    elseif(potential_type.eq.1) then!Straight Correlation
        call Vxi(qm2_struct,qm2ds,qmmm_struct,qm2ds%eta,v_solvent_difdens)
    endif
    v_solvent_difdens=linmixparam*v_solvent_difdens
    !Begin SCF loop
    do k=1,cosmo_scf_maxcyc
        call davidson(qm2_struct,qm2ds,qmmm_struct);        !Calculate new excited states
        if(qm2ds%verbosity.eq.5) call outDavidson(qm2_struct,qm2ds,qmmm_struct)
    ...
    call calc_excsolven(qm2ds,qmmm_struct,energy)
...
subroutine calc_cosmo_4(sim_target)
    ...
    if(solvent_model.eq.5) allocate(vsol_temp(sim%dav%nb,sim%dav%nb))
    ...
    call calc_rhotz(sim%qm2,sim%dav,sim%qmmm,sim%qmmm%state_of_interest,sim%dav%rhoTZ,calc_Z);
    call mo2sitef(sim%dav%Nb,sim%dav%vhf,sim%dav%rhoTZ,sim%dav%eta,sim%dav%tz_scratch);
    call packing(sim%dav%nb,sim%dav%eta,rhotzpacked_k, 's')
    ...
    if(solvent_model.eq.5) then !need solvent potential for excitation
        ...
        !Calculate Solvent Potential
        if(potential_type.eq.3) then !COSMO
            call VxiM(sim%dav,sim%dav%eta,v_solvent_difdens);
        elseif(potential_type.eq.2) then!ONSAGER
            call rcnfld(sim%qm2,sim%qmmm,v_solvent_difdens,sim%dav%eta,sim%dav%Nb)
        elseif(potential_type.eq.1) then!Straight Correlation
            call Vxi(sim%qm2,sim%dav,sim%qmmm,sim%dav%eta,v_solvent_difdens)
        endif
        v_solvent_difdens=linmixparam*v_solvent_difdens
    endif
    ...
    call mo2sitef(sim%dav%Nb,sim%dav%vhf,sim%dav%eta_tz,xi_k, &
    ...
    v_solvent_difdens(1:sim%qm2%norbs,1:sim%qm2%norbs)=0.d0;    !Clearing
    ...
    call calc_excsolven(sim%dav,sim%qmmm,energy)
    call calc_gssolven(sim%qm2,sim%dav,sim%qmmm,gsenergy)
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
subroutine calc_cosmo_3(qm2_struct,qm2ds,qmmm_struct)
    use qm2_davidson_module
    use qmmm_module,only:qm2_structure;
   ...
   _REAL_, DIMENSION(:), allocatable:: xi_k_1
    ...
    !Calculate Solvent Potential
    if(potential_type.eq.3) then!COSMO
        call VxiM(qm2ds,qm2ds%xi,v_solvent_xi);
    elseif(potential_type.eq.2) then!ONSAGER
        call rcnfld(qm2_struct,qmmm_struct,v_solvent_xi,qm2ds%xi,qm2ds%Nb)
    elseif(potential_type.eq.0) then!Straight Correlation
        call Vxi(qm2_struct,qm2ds,qmmm_struct,qm2ds%eta,v_solvent_xi)
    endif
    ...
    call davidson(qm2_struct,qm2ds,qmmm_struct);    !first davidson call with solvent potential
    ...
    do k=2,cosmo_scf_maxcyc
        ...
        elseif(potential_type.eq.2) then!ONSAGER
            call rcnfld(qm2_struct,qmmm_struct,v_solvent_xi,qm2ds%xi,qm2ds%Nb)
        elseif(potential_type.eq.0) then!Straight Correlation
            call Vxi(qm2_struct,qm2ds,qmmm_struct,qm2ds%xi,v_solvent_xi)
        endif
