...
subroutine runmd(xx,ix,ih,ipairs,x,winv,amass,f, &
   ...
   use sander_rism_interface, only: rismprm,rism_3d, RISM_NONE, RISM_FULL, RISM_INTERP, &
        rism_calc_type, rism_thermo_calc, rism_writesolvdistf, mylcm
#endif
   ...
   use remd, only : rem, mdloop, remd_ekmh, repnum, stagid, &
...
#  endif
   ...
   !AWG adaptive QM/MM
! SGLD
   use sgld, only : tsgld,trxsgld,tlangv,isgsta,isgend,rndfsg,fixcom, &
                    sgenergy,sgldw,sgmdw,sgfshake

! Should this be commented out? JMS
...
! for adaptive qm/mm runs
   ...
   _REAL_ :: adqmmm_first_energy
   logical :: flag_first_energy = .false.
   ...
   nr = nrp
   nr3 = 3*nr
   if (icnstph > 1) cphfirst_sol3 = cphfirst_sol * 3
   ekmh = 0.d0
...
#ifdef MPI
   if (mdloop .eq. 0 .and. master .and. relaxing == 0) then
#else
   if (master .and. relaxing == 0) then
#endif
      write (6,'(a,f8.0)') &
   ...
   !     Constant pH setup 
   !
   if (icnstph /= 0 .and. relaxing == 0 .and. mdloop .eq. 0) &
...
! Constant surface tension setup:
   ...
   ener       = null_state_rec !Zeros all these arrays
...
! 03132009            if( i_qi == 2 ) call qi_corrf_les ( cartpos, amass )
      ...
      if (icnstph /= 0 .and. relaxing == 0 .and. master .and. &
      ...
      if ( ifsc /= 0 ) then
         ...
         call mpi_reduce(sc_ener, sc_ener_tmp, 9, MPI_DOUBLE_PRECISION, &
                         MPI_SUM, 0, commsander, ierr)
         sc_ener(1:9) = sc_ener_tmp(1:9)
      end if
      if ( ifsc == 2 ) then
...
#ifdef MPI /* SOFT CORE */
      if ( ifsc /= 0 ) call calc_softcore_ekin(amass,v,v,istart,iend)
#endif
...
#  endif
   ...
   if (init == 4) then
      ...
      if ( rismprm%irism == 1 .and. rismprm%write_thermo==1 &
           .and. nstep <= 0 .and. facc /= 'A' .and. relaxing == 0) then
         if( rism_calc_type(0) == RISM_FULL)&
              call rism_thermo_calc()
      end if
#endif /*RISMSANDER*/
      if (nstep <= 0 .and. master .and. facc /= 'A' .and. relaxing == 0) then
         if(tsgld)CALL SGENERGY(ener)
         rewind(7)
#ifdef LES
         if (.not.ipimd.gt.0) &
           ener%tot = ener%kin%tot+ener%pot%tot
   ...
   ! Constant pH setup
   if (icnstph /= 0 .and. relaxing == 0 .and. &
   ...
   if ( icfe /= 0 )then
      ...
      if( master ) then
         ...
         if( onstep ) then
            ...
            if (ifsc == 1) then
               call adj_dvdl_stat(edvdl%pot%vdw, edvdl_r%pot%vdw)
               call adj_dvdl_stat(edvdl%pot%tot, edvdl_r%pot%tot)
            end if
         end if
...
! JMS -- comment out the adaptive QM/MM stuff since it was removed from the
! master branch
#  if 0
...
# endif /* LES */
   ...
   if (ntc /= 1) then
      ...
      if(tsgld)call SGFSHAKE(istart,iend,dtx,amass,x,.false.)   
      qspatial=.false.
      ...
      if(tsgld)call SGFSHAKE(istart,iend,dtx,amass,x,.true.)   
...
#  endif
      ...
      if (ifsc /= 0 ) call calc_softcore_ekin(amass,v,vold,istart,iend)
...
!
   ...
   if ( (onstep .and. relaxing == 0) .or. (aqmmm_flag > 0))then
      nvalid = nvalid + 1
      enert  = enert + ener
      ...
      if ( ifsc /= 0 ) then
         sc_ener_ave(1:9) = sc_ener_ave(1:9) + sc_ener(1:9)
         sc_ener_rms(1:9) = sc_ener_rms(1:9) + sc_ener(1:9)**2
      end if
#endif
...
!!FIX: TL - do we need to put in rismnrespa here?
   ...
   if(rismprm%irism==1)then
      ...
      if(rism_calc_type(nstep) == RISM_FULL .and. rismprm%write_thermo==1 &
           .and. lout)&
           call rism_thermo_calc()
   endif
#endif
...
#  ifdef MPI
         ...
         if ( qmmm_nml%vsolv > 1 ) then
            ...
            if ( masterrank == 0 ) then
               ...
               if (aqmmm_flag > 0 .and. nstep-1 > aqmmm_flag) then

                  ! print total energy for adaptive qm/mm into a separate file
                  ! when qmmm_vsolv%verbosity > 0
                  if (qmmm_vsolv%verbosity > 0) then
                     if (.not. flag_first_energy) then
                        flag_first_energy = .true.
                        adqmmm_first_energy = corrected_energy+kinetic_E_save(aqmmm_flag)
                     endif

                     if (flag_first_energy) then
                        open(80,file='adqmmm_tot_energy.dat',position='append')
                        write(80,'(i9,5x,f10.3)')nstep-aqmmm_flag+1, &
                          corrected_energy+kinetic_E_save(aqmmm_flag)-adqmmm_first_energy
                        close(80)
                     end if
                  end if

                  write(6,'(a,i9)')'Adaptive QM/MM energies at step', &
                                      nstep - aqmmm_flag + 1
                  write(6,'(a,f20.5)')'Adaptive QM/MM : E_tot_corr =', &
                                      corrected_energy + kinetic_E_save(aqmmm_flag)
                  write(6,'(a,f20.5)')'Adaptive QM/MM : E_pot_corr =', &
                                      corrected_energy
               end if
            end if
         end if
...
!     if (mdloop>0) mytemp = ener%kin%tot * onefac(1)
      mytemp = ener%kin%tot * onefac(1)
      myeptot = ener%pot%tot
...
! ------====== END REMD Post-Dynamics ======------
   ...
   if (relaxing == 1) return  ! don't print the ending stuff if we're just relaxing waters
   !=======================================================================
   !     ----- PRINT AVERAGES -----
   !=======================================================================
