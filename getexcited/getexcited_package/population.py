...
def population(header):
    ...
    if ctraj == 0:
        ...
        for tstep in np.arange(tscol):
            output.wirte( '%0*.2f' % (len(str((header.n_class_steps))) + 2,header.time_step*tstep), ' '.join(str('%.3f' % (x)) for x in fpoph[tstep]), '%.3f' % (np.sum(fpoph[tstep])), ' '.join(str('%.3f' % (x)) for x in fpopc[tstep]), '%.3f' % (np.sum(fpopc[tstep])))
    if errflag == 1:
