...
def nexmd(header):
    ...
    if len(NEXMDs) != 0:
        contq = eval(
            input(
                "** WARNING ** All NEXMD folders and rseedslists inside %s will be"
                " deleted!\ndo you want to continue? Answer yes [1] or no [0]: "
                % (outdir)
            )
        )
        if contq not in [1, 0]:
    ...
    if header.n_class_steps <= 0:
        print(
            "Must change n_class_steps in %s/header to greater than 0 for dynamics."
            % (outdir)
        )
        sys.exit()
    ...
    if state_set == 0 and coeff_set == 1:
        print(
            "There is an inconsistency in header.\nInput exc_state_init is not set,"
            " while coefficients are set."
        )
        sys.exit()
    if state_set == 0 and coeff_set == 0:
        print(
            "Initial excited states will model a photoexcited wavepacket according to"
    ...
    if state_set == 1 and coeff_set == 1 or state_set == 1 and coeff_set == 0:
        print(
            "A total of %d coordinate files, ranging from %.2f to %.2f fs in increments"
            " of %.2f fs, were found." % (ncoords, tinit, time, tinc)
        )
    if state_set == 0 and coeff_set == 0:
        print(
            "A total of %d coordinate files, ranging from %.2f to %.2f fs in increments"
            " of %.2f fs, were found.\nNote: only coordinate files used for"
            " single-point calculations can be used for NEXMD."
            % (ncoords, tinit, time, tinc)
        )
    ...
    if state_set == 0 and coeff_set == 0:
        ...
        ntrajq = eval(
            input(
                "How many trajectories for NEXMD? Enter a number no greater than %d: "
                % (ntraj)
            )
        )
        ...
        if ntrajq < 0:
            ...
            coordsq = eval(
                input(
                    "You have requested %d randomly-selected coordinate files in the"
                    " range %d to %d.\nContinue? Answer yes [1] or no [0]: "
                    % (ntraj, dirlist[0], dirlist[-1])
                )
            )
        else:
            ...
            coordsq = eval(
                input(
                    "You have requested %d evenly-spaced coordinate files in the range"
                    " %d to %d for NEXMD.\nContinue? Answer yes [1] or no [0]: "
                    % (ntraj, dirlist1[0], dirlist1[0::interval][-1])
                )
            )
    ...
    if state_set == 1 and coeff_set == 1 or state_set == 1 and coeff_set == 0:
        coords = eval(
            input(
                ...
                " and the number of trajectories.\nInput an array of the form [start,"
                " end, number]: "
            )
        )
        if not isinstance(coords, list):
            print(
                "Input must be an array of the form [start, end, number].\nFor example,"
                " [1, 1000, 500] requests 500 coordinate files sampled from 1 to 1000."
            )
            sys.exit()
        if len(coords) != 3:
            print(
                "Input must be an array with three elements of the form [start, end,"
                " number].\nFor example, [1, 1000, 500] requests 500 coordinate files"
                " sampled from 1 to 1000."
            )
            sys.exit()
        index = 0
        for i in coords:
            if isinstance(i, int) == False:
                print(
                    "Element number %d of input array must be integer.\nUser inputted"
                    " [%s, %s, %s], which is not allowed."
                    % (index + 1, coords[0], coords[1], coords[2])
                )
                sys.exit()
            if index in [0, 1]:
                if i not in np.arange(ncoords):
                    print(
                        "Element number %d of input array must be integer between 0 and"
                        " %d.\nUser inputted [%d, %d, %d], which is not allowed."
                        % (index + 1, ncoords - 1, coords[0], coords[1], coords[2])
                    )
                    sys.exit()
            else:
                if i not in np.arange(1, ncoords + 1) and i not in -np.arange(
                    1, ncoords + 1
                ):
                    print(
                        "Element number %d of input array must be integer between 1 and"
                        " %d.\nUser inputted [%d, %d, %d], which is not allowed."
                        % (index + 1, ncoords, coords[0], coords[1], coords[2])
                    )
                    sys.exit()
            index += 1
        if coords[0] > coords[1]:
            print(
                "Second element of input array must be greater than first"
                " element.\nUser inputted [%d, %d, %d], which is not allowed."
        ...
        if (coords[1] - coords[0]) + 1 < np.abs(coords[2]):
            print(
                "Number of coordinate files requested must be less than or equal to the"
                " number of coordinate files in the sample.\nUser inputted [%d, %d,"
                " %d], which is not allowed." % (coords[0], coords[1], coords[2])
            )
            sys.exit()
        if coords[2] < 0:
            ...
            coordsq = eval(
                input(
                    "You have requested %d randomly-selected coordinate files in the"
                    " range %d to %d.\nContinue? Answer yes [1] or no [0]: "
                    % (ntraj, coords[0], coords[1])
                )
            )
        else:
            interval = np.int(
                np.ceil((coords[1] - coords[0] + 1) / np.float(coords[2]))
            )
    ...
    if split <= 0:
        print(
            "Number of trajectories per NEXMD folder must be integer greater than zero."
        )
        sys.exit()
    ...
    if state_set == 1 and coeff_set == 1 or state_set == 1 and coeff_set == 0:
        ...
        if coords[2] < 0:
            ...
            dirsplit = np.split(
                np.sort(random.sample(np.arange(coords[0], coords[1] + 1), ntraj)),
                dirsplit,
            )
        else:
            dirsplit = dirsplit.astype(np.int64)
    ...
    if randq == 1:
        ...
        if lenrseeds < ntraj:
            print(
                ...
                " %d, while the number of trajectories requested is %d."
                % (lenrseeds, ntraj)
            )
            sys.exit()
    for rseed in rseeds:
        if rseed < 0:
            print(
                ...
                " getexcited_package, a negative seed is assigned to a trajectory that"
                " could not be prepared due to some problem." % (rseed)
            )
            sys.exit()
    ...
    if state_set == 0 and coeff_set == 0:
        ...
        stype = eval(
            input("Spectral lineshape? Answer Gaussian [0] or Lorentzian [1]: ")
        )
        if stype not in [0, 1]:
        ...
        if stype == 0:
            FWHM = eval(
                input(
                    "Please enter the full width at half maximum (FWHM) for the"
                    " Gaussian lineshape in eV [e.g. 0.36]: "
                )
            )
            ...
            FWHM = eval(
                input(
                    "Please enter the full width at half maximum (FWHM) for the"
                    " Lorentzian lineshape in eV  [e.g. 0.36]: "
                )
            )
            specb = FWHM
        ...
        for NEXMD in np.arange(1, np.ceil(np.float(ntraj) / split) + 1):
            ...
            for dir in dirsplit[index]:
                ...
                if stype == 0:
                    for state, energy, osx, osy, osz, oscstren in data:
                        qpop[oindex] = (
                            oscstren
                            * np.exp(
                                -((energy - excen) ** (2.0)) / (2.0 * specb ** (2.0))
                            )
                            / np.sqrt(2.0 * np.pi * specb ** (2.0))
                        )
                        oindex += 1
                else:
                    for state, energy, osx, osy, osz, oscstren in data:
                        qpop[oindex] = (
                            oscstren
                            / (1.0 + ((energy - excen) ** 2.0) / (specb / 2.0) ** (2.0))
                            / (specb * np.pi / 2.0)
                        )
                        oindex += 1
                ...
                inputfile = open(
                    "%s/NEXMD%d/%04d/input.ceon" % (outdir, NEXMD, dir), "w"
                )
            ...
            dirlist.close()
            shutil.copyfile(
                "%s/NEXMD%d/dirlist" % (outdir, NEXMD),
                "%s/NEXMD%d/dirlist1" % (outdir, NEXMD),
            )
            index += 1
