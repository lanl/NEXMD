...
def restart(pathtopack, header):
    ...
    numRestarts = eval(
        input(
            "Input the number of times you have restarted the trajectories? e.g 1 if"
            " this is the first time you have run a restart:"
        )
    )
    ...
    if header.out_data_steps == 0:
        print(
            "No data have been printed to files because out_data_steps = 0 in"
            " %s/header." % (NEXMDir)
        )
        sys.exit()
    ...
    print(
        "Currently, trajectories are set to run for %d classical steps with a time-step"
        " of %.2f fs.\nThis is a total of %.2f fs."
        % (
            header.n_class_steps,
            header.time_step,
            header.n_class_steps * header.time_step,
        )
    )
    ...
    if tsmaxq == 0:
        ...
        if ntsmax <= header.n_class_steps:
            print(
                "Answer must be greater than or equal to the previous number of"
                " classical steps used, which was %d.\nTo reduce number of classical"
                " steps past %d, simply change n_class_steps in header."
                % (header.n_class_steps, header.n_class_steps)
            )
            sys.exit()
        ...
        for line in headerFile:  # Changed
            if "n_class_steps" in line:
                nheader.write(
                    "   n_class_steps=%d, ! number of classical steps [1]\n" % (ntsmax)
                )
            else:
                nheader.write(line)
    ...
    if randq == 0:
        rseeds = input(
            "Path to random-seeds list (** must be different from past random seeds"
            " **): "
        )
        ...
        if lenrseeds < ntraj:
            print(
                "Length of random-seeds list must be equal to or greater than the"
                " number of trajectories.\nUser inputted a random-seeds list of length"
                " %d, while the number of trajectories is %d." % (lenrseeds, ntraj)
            )
            sys.exit()
        for rseed in rseeds:
            if rseed < 0:
                print(
                    "A negative random seed was detected, %d.\nWithin the"
                    " getexcited_package, a negative seed is assigned to a trajectory"
                    " that could not be prepared due to some problem." % (rseed)
                )
                sys.exit()
        ...
        for NEXMD in NEXMDs:
            ...
            for dir in dirlist1:
                if not os.path.exists("%s/%04d/energy-ev.out" % (NEXMD, dir)):
                    print(
                        "Path %s%04d/energy-ev.out does not exist." % (NEXMD, dir),
                        file=error,
                    )
                ...
                if tsteps != header.n_class_steps:
                    if not os.path.exists("%s/%04d/restart.out" % (NEXMD, dir)):
                        print(
                            "Path %s/%04d/restart.out does not exist." % (NEXMD, dir),
                            file=error,
                        )
                        print("%d" % (-123456789), file=rseedslist)
                        rstflag = 1
                    ...
                    if len(array) != 6:
                        print(
                            "Path %s%04d/restart.out is incomplete." % (NEXMD, dir),
                            file=error,
                        )
                        print("%d" % (-123456789), file=rseedslist)
                        rstflag = 1
                    ...
                    ## Make new input file ##
                    os.rename(
                        "%s/%04d/input.ceon" % (NEXMD, dir),
                        "%s/%04d/input%d.ceon" % (NEXMD, dir, numRestarts),
                    )
                    ...
                    for line in headerFile:  # Changed
                        if "rnd_seed_flag" in line:
                            inputfile.write(
                                "   rnd_seed=%d, ! seed for the random number"
                                " generator\n" % (rseed if randq == 1 else rseeds[traj])
                            )
                        else:
                            if "exc_state_init_flag" in line:
                                inputfile.write(
                                    "   exc_state_init=%d, ! initial excited state (0 -"
                                    " ground state) [0]\n" % (state)
                                )
                            else:
                                if "time_init" in line:
                                    inputfile.write(
                                        "   time_init=%.1f, ! initial time, fs [0.00]\n"
                                        % (time)
                                    )
                                else:
                                    if "n_class_steps" in line:
                                        inputfile.write(
                                            "   n_class_steps=%d, ! number of classical"
                                            " steps [1]\n"
                                            % (
                                                np.int(
                                                    header.n_class_steps
                                                    - time / header.time_step
                                                )
                                            )
                                        )
                                        ...
                                        if "out_count_init" in line:
                                            inputfile.write(
                                                "   out_count_init=%d, ! initial count"
                                                " for output files [0]\n" % (maxview)
                                            )
                                        else:
                    ...
                    print(
                        "%d" % (rseed if randq == 1 else rseeds[traj]), file=rseedslist
                    )
                else:
                    print("%d" % (-123456789), file=rseedslist)
                traj += 1
    ...
    if dynq == 1:  ## single trajectory
        ...
        if tsteps != header.n_class_steps:
            ...
            ## Make new input file ##
            os.rename(
                "%s/input.ceon" % (NEXMDir), "%s/input%d.ceon" % (NEXMDir, numRestarts)
            )
            ...
            for line in headerFile:  # Changed
                if "rnd_seed_flag" in line:
                    inputfile.write(
                        "   rnd_seed=%d, ! seed for the random number generator\n"
                        % (rseed if randq == 1 else rseeds[traj])
                    )
                else:
                    if "exc_state_init_flag" in line:
                        inputfile.write(
                            "   exc_state_init=%d, ! initial excited state (0 - ground"
                            " state) [0]\n" % (state)
                        )
                    else:
                        if "time_init" in line:
                            ...
                            if "n_class_steps" in line:
                                inputfile.write(
                                    "   n_class_steps=%d, ! number of classical steps"
                                    " [1]\n"
                                    % (
                                        np.int(
                                            header.n_class_steps
                                ...
                                if "out_count_init" in line:
                                    inputfile.write(
                                        "   out_count_init=%d, ! initial count for"
                                        " output files [0]\n" % (maxview)
                                    )
                                else:
                                    if "nucl_coord_veloc_flag" in line:
        ...
        traj += 1
        if filecmp.cmp(
            "%s/rseedslist%d" % (NEXMDir, maxdir),
            "%s/rseedslist%d" % (NEXMDir, maxdir + 1),
        ):
            os.remove("%s/rseedslist%d" % (NEXMDir, maxdir + 1))
    ...
    contq = eval(
        input(
            "Continue to delete extraneous data in output files? Answer yes [1] or no"
            " [0]: "
        )
    )
    if contq not in [1, 0]:
    ...
    if header.bo_dynamics_flag == 0:
        ...
        files = np.array(
            ["energy-ev.out", "temperature.out", "pes.out", "transition-densities.out"]
        )
        ofiles = np.array([])
    ...
    if dynq == 0:  ## ensemble
        ...
        for NEXMD in NEXMDs:
            ...
            for dir in dirlist:
                if not os.path.exists("%s/%s/%04d" % (cwd, NEXMD, dir)):
                    print(
                        "Path %s/%s%04d does not exist." % (cwd, NEXMD, dir), file=error
                    )
                    rstflag = 1
                    traj += 1
                    continue
                ...
                for index in np.arange(len(files)):
                    if not os.path.exists(
                        "%s/%s/%04d/%s" % (cwd, NEXMD, dir, files[index])
                    ):
                        print(
                            "Path %s/%s%04d/%s does not exist."
                            % (cwd, NEXMD, dir, files[index]),
                            file=error,
                        )
                        rstflag = 1
                        continue
                    ...
                    data = subprocess.check_output(
                        ["tail", "-1", "%s" % (files[index])]
                    )
                    ltime = np.around(
                        np.float(
                            np.fromstring(data, dtype=float, sep=" ")[
                                1 if index == 0 and header.bo_dynamics_flag == 0 else 0
                            ]
                        ),
                        decimals=3,
                    )
                    if rtimes[traj] > ltime + header.out_data_steps * header.time_step:
                        print(
                            "last time-step in",
                            "%s/%s%04d/%s" % (cwd, NEXMD, dir, "restart.out"),
                            "exceeds last time-step in",
                            "%s/%s%04d/%s" % (cwd, NEXMD, dir, files[index]),
                            file=error,
                        )
                        rstflag = 1
                    ...
                    while (
                        rtimes[traj]
                        + ncsteps * (header.out_data_steps * header.time_step)
                        <= ltime
                    ):
                        ncsteps += 1
                    if header.moldyn_verbosity == 3 and index in [2, 4]:
                        lctime = rtimes[traj] + (ncsteps - 1) * (
                            header.out_data_steps * header.time_step
                        )
                        nqsteps = 0
                        while (
                            lctime * header.n_quant_steps + nqsteps * header.time_step
                            <= ltime * header.n_quant_steps
                        ):
                            nqsteps += 1
                        if ltime == lctime:
                            nlines = (
                                (ncsteps - 1)
                                * (
                                    header.out_data_steps * (header.n_quant_steps - 1)
                                    + 1
                                )
                                + (nqsteps - 1)
                                + 1
                    ...
                    if header.bo_dynamics_flag == 0:
                        subprocess.call(
                            shlex.split(
                                "sh %s/getexcited_package/cutdata.sh %d %s"
                                % (
                                    pathtopack,
                                    (nlines - 1 if index in [2, 3, 5] else nlines),
                                    files[index],
                                )
                            )
                        )
                        ...
                        subprocess.call(
                            shlex.split(
                                ...
                                % (
                                    pathtopack,
                                    (nlines - 1 if index == 1 else nlines),
                                    files[index],
                                )
                            )
                        )
                ...
                for index in np.arange(len(ofiles)):
                    if not os.path.exists(
                        "%s/%s/%04d/%s" % (cwd, NEXMD, dir, ofiles[index])
                    ):
                        print(
                            "Path %s/%s%04d/%s does not exist."
                            % (cwd, NEXMD, dir, ofiles[index]),
                            file=error,
                        )
                        rstflag = 1
                        continue
                    ...
                    while ltime >= rtimes[traj]:
                        if os.stat(ofiles[index]).st_size == 0:
                            break
                        data = subprocess.check_output(
                            ["tail", "%d" % (-(nlines + 1)), "%s" % (ofiles[index])]
                        )
                    ...
                    subprocess.call(
                        shlex.split(
                            "sh %s/getexcited_package/cutdata.sh %d %s"
                            % (pathtopack, nlines - 1, ofiles[index])
                        )
                    )
        ...
        if rstflag == 1:
            print(
                "One or more trajectories cannot be restarted properly, check"
                " delextra.err."
            )
        else:
    ...
    if dynq == 1:  ## single trajectory
        ...
        for index in np.arange(len(files)):
            if not os.path.exists("%s/%s/%s" % (cwd, NEXMDir, files[index])):
                print(
                    "%s/%s/%s does not exist." % (cwd, NEXMDir, files[index]),
                    file=error,
                )
                rstflag = 1
                continue
            ...
            if rtimes[traj] > ltime + header.out_data_steps * header.time_step:
                print(
                    "last time-step in",
                    "%s/%s/%s" % (cwd, NEXMDir, "restart.out"),
                    "exceeds last time-step in",
                    "%s/%s/%s" % (cwd, NEXMDir, files[index]),
                    file=error,
                )
                rstflag = 1
                continue
            ncsteps = 0
            while (
                rtimes[traj] + ncsteps * (header.out_data_steps * header.time_step)
                <= ltime
            ):
                ncsteps += 1
            if header.moldyn_verbosity == 3 and index in [2, 4]:
                ...
                if ltime == lctime:
                    nlines = (
                        (ncsteps - 1)
                        * (header.out_data_steps * (header.n_quant_steps - 1) + 1)
                        + (nqsteps - 1)
                        + 1
                    )
                else:
        ...
        for index in np.arange(len(ofiles)):
            if not os.path.exists("%s/%s/%s" % (cwd, NEXMDir, ofiles[index])):
                print(
                    "%s/%s/%s" % (cwd, NEXMDir, ofiles[index]),
                    "does not exist",
                    file=error,
                )
                rstflag = 1
