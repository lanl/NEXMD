...
def newsim(header):
    print(
        "Preparing input files for a new simulation with geometries taken from another"
        " simulation."
    )
    ...
    if not os.path.exists(gsdir):
        ...
        sys.exit()
    olddir = input("NEXMD directory where geometries should be taken from: ")
    ...
    if len(NEXMDs) != 0:
        contq = eval(
            input(
                "** WARNING ** All NEXMD folders and rseedslist inside %s will be"
    ...
    if old_header.out_coords_steps == 0:
        print(
            "No coordinates have been printed to coords.xyz because out_coords_steps ="
            " 0 in %/header." % (olddir)
        )
        sys.exit()
    ...
    cprint = (
        old_header.time_step * old_header.out_data_steps * old_header.out_coords_steps
    )
    print(
        "The coordinates in %s began at %.2f fs and were printed to coords.xyz every"
        " %.2f fs." % (olddir, old_header.time_init, cprint)
    )
    ...
    if new_header.n_class_steps <= 0:
        print(
            "Must change n_class_steps in %s/header to greater than 0 to run dynamics."
            % (newdir)
        )
        sys.exit()
    ...
    startq = eval(
        input(
            "At what time, in femtoseconds, should geometries be taken from %s?  Plese"
            " enter a time greater than 0:   " % (olddir)
        )
    )
    ...
    if round((nsteps - 1) * cprint + old_header.time_init, 2) != round(startq, 2):
        print(
            "The time, %.2f, minus the initial time, %.2f, is not divisible by %.2f."
            % (startq, old_header.time_init, cprint)
        )
        sys.exit()
    ...
    for NEXMD in NEXMDs:
        ...
        print("%s/dirlist1" % (NEXMD))
        if isinstance(dirlist1, int) == True:
            dirlist1 = np.array([dirlist1])
        for dir in dirlist1:
            if not os.path.exists("%s/%04d/coords.xyz" % (NEXMD, dir)):
                print("%s/%04d/coords.xyz" % (NEXMD, dir), "does not exist", file=error)
                errflag = 1
                terrflag = 1
            if not os.path.exists("%s/%04d/velocity.out" % (NEXMD, dir)):
                print(
                    "%s/%04d/velocity.out" % (NEXMD, dir), "does not exist", file=error
                )
                errflag = 1
                terrflag = 1
            if errflag == 1:
                ...
                if "time" in line:
                    if ncoords == 0:
                        ...
                        if dtinit != old_header.time_init:
                            print(
                                "Initial time in %s%04d/coords.xyz does not match"
                                " initial time in %s/header." % (NEXMD, dir, olddir),
                                file=error,
                            )
                            errflag = 1
                            terrflag = 1
            ...
            try:
                ...
                print(
                    " There is problem with the directory: %s%04d" % (NEXMD, dir),
                    file=error,
                )
                traj += 1
                continue
            if len(cindex[0]) == 0:
                print(
                    "Coordinates at %.2f fs in %s%04d/coords.xyz were not found."
                    % (startq, NEXMD, dir),
                    file=error,
                )
                tarrayc = np.append(tarrayc, np.array([-9999, -9999]))
            ...
            if len(vindex[0]) == 0:
                print(
                    "Velocities at %.2f fs in %s%04d/velocity.out were not found."
                    % (startq, NEXMD, dir),
                    file=error,
                )
                tarrayc = np.append(tarrayc, np.array([-9999, -9999]))
    ...
    if terrflag == 1:
        print(
            "One or more trajectories of the previous simulation cannot be used for in"
            " the current simulation, check newsim.err."
        )
        contq = eval(
            input("Continue preparing input files? Answer yes [1] or no [0]: ")
        )
        if contq not in [1, 0]:
            print("Answer must be 1 or 0.")
            sys.exit()
    ...
    for NEXMD in np.arange(1, len(NEXMDs) + 1):
        ...
        for dir in dirlist1:
            ...
            if -9999 in tarrayc[traj]:
                print(
                    "The input file for %s/NEXMD%d/%04d cannot be generated."
                    % (olddir, NEXMD, dir),
                    file=error,
                )
                errflag = 1
        ...
        dirlist.close()
        shutil.copyfile(
            "%s/NEXMD%d/dirlist" % (newdir, NEXMD),
            "%s/NEXMD%d/dirlist1" % (newdir, NEXMD),
        )
