...
def spcalc(header):
    ...
    if len(NEXMDs) != 0:
        contq = eval(
            input(
                "** WARNING ** All NEXMD folders inside %s will be deleted!\nContinue?"
                " Answer yes [1] or no [0]: " % (outdir)
            )
        )
        if contq not in [1, 0]:
    ...
    if header.n_class_steps != 0:
        print(
            "User must change n_class_steps in %s/header to 0 for single-point"
            " calculations." % (outdir)
        )
        sys.exit()
    ...
    tinc = time / (ncoords - 1)
    print(
        "A total of %d coordinates files, ranging from %.2f to %.2f fs in increments of"
        " %.2f fs, were found." % (ncoords, tinit, time, tinc)
    )

    ## Choose geometries ##
    coords = eval(
        input(
            "Enter requested range of the ground-state sampling by coordinate files and"
            " the number of single-point calculations.\nInput an array of the form"
            " [start, end, number]: "
        )
    )
    if not isinstance(coords, list):
        print(
            ...
            " 1000, 500] requests 500 coordinate files sampled from 1 to 1000."
        )
        sys.exit()
    if len(coords) != 3:
        print(
    ...
    if coords[0] > coords[1]:
        print(
            "second element of input array must be greater than first element.\nUser"
            " inputted [%d, %d, %d], which is not allowed."
    ...
    if (coords[1] - coords[0]) + 1 < np.abs(coords[2]):
        print(
            ...
            " which is not allowed." % (coords[0], coords[1], coords[2])
        )
        sys.exit()
    if coords[2] < 0:
        ...
        coordsq = eval(
            input(
                "You have requested %d randomly-selected coordinate files in the range"
        ...
        coordsq = eval(
            input(
                "You have requested %d evenly-spaced coordinate files in the range %d"
                " to %d.\nContinue? Answer yes [1] or no [0]: "
                % (ntraj, coords[0], coords[1])
            )
        )
    ...
    ## Split geometries ##
    split = eval(
        input("Number of single-point calculations per NEXMD folder [e.g. 100]: ")
    )
    ...
    if split < 0:
        print(
            "Number of single-point calculations per NEXMD folder must be integer"
            " greater than zero."
        )
        sys.exit()
    ...
    if coords[2] < 0:
        dirsplit = np.split(
            np.sort(random.sample(np.arange(coords[0], coords[1] + 1), ntraj)), dirsplit
        )
    else:
