...
def bla(header):
    ...
    if dynq == 1: ## single trajectory
        ...
        if ctraj == 0:
            ...
            for ncoord in np.arange(ncoords):
                print >> output, '%0*.2f' % (len(str((header.n_class_steps))) + 2,header.time_step*header.out_data_steps*header.out_coords_steps*ncoord), '%08.3f' % (sbla[ncoord])
    ...
    if dynq == 0 and typeq == 0: ## mean from ensemble
        ...
        if ctraj == 0:
            ...
            print >> output, 'excellent trajectories: ', '%04d' % (etraj)
            for ncoord in np.arange(ncoords):
                print >> output, '%0*.2f' % (len(str((header.n_class_steps))) + 2,header.time_step*header.out_data_steps*header.out_coords_steps*ncoord), '%08.3f' % (fbla[ncoord]), '%07.3f' % (ebla[ncoord])
        if errflag == 1:
