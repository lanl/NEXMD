...
R=$(echo "scale=6; e( -1/3 * l($l*$stepx) )" | bc -l) ##Calculate the new Ex values
x=$(echo "scale=6; $l*$stepx " | bc -l)

echo "step $x E_$x.out R=$R"

sed "s\onsager_radius=[\.0-9]*\onsager_radius=$R\g" -i input.ceon ##Replace Ex value in input file 
